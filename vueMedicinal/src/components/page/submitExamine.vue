    ...
    <div>
        <div>
            <div style="display: flex;justify-content: space-between">
                ...
                <div>
                    <el-button type="primary" icon="el-icon-plus" @click="showAddEmpView">提交审核</el-button>
                </div>
            </div>
        </div>
                    ...
                    <el-row >
                        ...
                        <el-col :span="6">
                            <el-form-item label="审核人:" prop="examineuserid">
                                 <el-select v-model="emp.examineuserid" placeholder="请选择">
                                    <el-option
...
export default {
    ...
    methods: {
        ...
        deleteEmp(data) {
            this.$confirm('此操作将永久删除【' + data.nname + '】, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            })
                .then(() => {
                    this.deleteRequest('/drug/' + data.nid).then(resp => {
                        if (resp) {
                            this.initEmps();
                        }
                    });
        ...
        doAddEmp() {
                this.$refs['empForm'].validate(valid => {
                    if (valid) {
                        ...
                        this.postRequest('/drug', this.emp).then(resp => {
                            if (resp) {
        ...
        showAddEmpView() {
            ...
            this.title = '添加药品';
            this.dialogVisible = true;
            if (!window.sessionStorage.getItem("examine")) {
                this.getRequest('/examineUser').then(resp => {
                    if (resp) {
                        this.examine = resp;
                        window.sessionStorage.setItem("examine", JSON.stringify(resp));
                    }
                })
            } else {
                this.examine = JSON.parse(window.sessionStorage.getItem("examine"));
            }
        },
        initSelectedMenus() {
        ...
        initEmps(type) {
            this.loading = true;
            let url = '/drug?page=' + this.page + '&size=' + this.size;
